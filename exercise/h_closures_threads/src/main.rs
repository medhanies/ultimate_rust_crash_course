...
fn main() {
    ...
    // the child threads.
    let foo_a = thread::spawn(move || {
        pause_ms(0);
        tx.send("Thread One: 1").unwrap();
        pause_ms(200);
        tx.send("Thread One: 2").unwrap();
    });

    pause_ms(100); // Make sure Thread A has time to get going before we spawn Thread B

    let foo_b = thread::spawn(move || {
        pause_ms(0);
        tx2.send("Thread Two: 1").unwrap();
        pause_ms(200);
        tx2.send("Thread Two: 2").unwrap();
    });
